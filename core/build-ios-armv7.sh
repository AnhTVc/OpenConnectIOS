...
buildIOS(){
	...
	LDFLAGS="$LDFLAGS -arch armv7 -fPIE -miphoneos-version-min=${MINIOSVERSION} ${EXTRA_LDFLAGS} -L${OUTPUTDIR}/lib" \
	CFLAGS="$CFLAGS -g -O0 -D__APPLE_USE_RFC_3542 -arch armv7 -fPIE -miphoneos-version-min=${MINIOSVERSION} ${EXTRA_CFLAGS} -I${OUTPUTDIR}/include -isysroot ${DEVELOPER}/Platforms/${PLATFORM}.platform/Developer/SDKs/${PLATFORM}${SDKVERSION}.sdk" 
...
make install
#make clean
lipo -arch armv7 /usr/local/lib/libopenconnect.a -create -output /usr/local/lib/libopenconnect.
