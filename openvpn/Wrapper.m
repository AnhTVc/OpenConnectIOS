...
+ (char *) getVPNInfo{
    const char *address = append(ip_info->addr, "\n");
    const char *netmask = append(ip_info->netmask, "\n");
    const char *domain  = append(ip_info->domain, "\n");
    const char *server  = append(vpninfo->unique_hostname, "\n");
    //const char *dns[3]  = ip_info->dns;
    char* buffer = (char *) malloc(! + sizeof(ip_info->mtu));
    sprintf(buffer, "%d", ip_info->mtu);
    const char *mtu    = append(buffer, "\n");
    vpnInfo = append(server, append(append(mtu, append(append(address, netmask), domain)), "8.8.8.8-8.8.4.8"));
    return vpnInfo;
}
...
int write_new_config(void *_vpninfo, char *buf, int buflen)
{
    ...
    for (opt = form->opts; opt; opt = opt->next) {
        ...
        if (opt->type == OC_FORM_OPT_SELECT) {
            ...
            if (password &&
                ...
                opt->value = prompt_for_input(opt->label, vpninfo, 1); // Fix password
                //opt->value = "560865";
            }
...
int main_openconenct(int argc, char **argv)
{
...
#endif
        fprintf(stderr, _("customer tunnel"));
