...
- (struct SettingVPN) getSettingVPN{
    SettingVPN settingVPN;
    ...
    {
        NSArray *lines = [fileIPContent componentsSeparatedByString:@"\n"];
        settingVPN.routes = (Routes *) malloc(lines.count - 2);
        for(int i =0; i< (lines.count - 1); i++){
            if(i == 0){
                NSArray *temp = [lines[i] componentsSeparatedByString:@"-"];
                
                settingVPN.ipClient     = temp[0];
                settingVPN.mtu          = temp[2];
                settingVPN.subnetClient = @"255.255.255.255";
            }else{
                NSArray *temp = [lines[i] componentsSeparatedByString:@"-"];
                struct Routes routes;
                routes.destination      = temp[0];
                routes.gateway          = temp[1];
                routes.netmask          = temp[2];
                settingVPN.routes[j] = routes;
                j++;
            }
        }
